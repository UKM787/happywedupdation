...
<div class="d-flex justify-content-around">
    <div>
        <a class="btn wed-btn-main" href="{{route('adminservice.index')}}">Services Categories</a>
    </div>
    <div>
        <a class="btn wed-btn-main" href="{{ route('adminservice.create') }}"><i class="material-icons" title="Add new Vendor">add</i></a>
    </div>
    <div>
...
<div class="row g-3 my-2">
    ...
    <div class="col-md-4">
        <form id="cityfilters" action="{{route('adminservicefilter')}}" method="GET">

            @csrf
            <select name="categoryId" id="categoryId" class="form-select" onchange="document.querySelector('#cityfilters').submit();">
                <option value=""> activeServices</option>
                @foreach ($activeServices as $category)
                <option value="{{ $category->id }}">{{ $category->name }}</option>
                @endforeach
            </select>
        </form>
...
<x-servicefilter :services="$services" :items="$items" />
